...
static void calc_orientations( std::vector<godot::Transform3D> &transforms,
                               const float segment_length )
{
    for ( size_t i = 0; i < transforms.size(); ++i )
    {
        ...
        {
            trans.basis =
                prev_trans.basis.rotated( a, -prev_trans.basis.get_column( 1 ).angle_to( u ) );
        }
        else
        {
...
static void connect_strips_with_same_point_count( std::vector<int> &indices, int prev_strip_begin,
                                                  int point_count )
{
...
static void connect_strips_with_different_point_count( std::vector<int> &indices,
                                                       int prev_strip_begin, int prev_point_count,
                                                       int next_strip_begin, int next_point_count )
{
...
static void add_cap( TG_SurfaceData &surface, const TG_SurfaceData &base_surface,
                     const godot::Transform3D &trans, int point_count, bool flat, float radius )
{
...

    {
        ...
        for ( int pi = 0; pi < point_count; ++pi )
        {
            ...
            if ( flat )
            {
                ...
                const TG_Tangents t = get_tangents_from_axes( trans.basis.get_column( 1 ),
                                                              trans.basis.get_column( 0 ),
                                                              trans.basis.get_column( 2 ) );
    ...
    {
        const TG_Tangents t = get_tangents_from_axes(
            trans.basis.get_column( 1 ), trans.basis.get_column( 0 ), trans.basis.get_column( 2 ) );
        surface.tangents.push_back( t );
    }
...
static TG_SurfaceData &get_or_create_surface( std::vector<TG_SurfaceData> &surfaces,
                                              int surface_index )
{
...
static void generate_path_mesh( std::vector<TG_SurfaceData> &surfaces,
                                const std::vector<godot::Transform3D> &transforms,
                                const std::vector<float> &radii, const std::vector<float> distances,
                                float mesh_divisions_per_unit, bool end_cap_flat,
                                int main_material_index, int cap_material_index,
                                godot::Vector2 uv_scale, bool constant_divisions )
{
    ...
    // Main path
    {
        ...
        const int base_point_count =
            max( static_cast<int>( base_circumference * mesh_divisions_per_unit ), 3 );
        ...
        for ( size_t transform_index = 0; transform_index < transforms.size(); ++transform_index )
        {
            ...
            for ( int pi = 0; pi < point_count; ++pi )
            {
                ...
                const godot::Vector3 normal =
                    trans.basis.get_column( 0 ).rotated( trans.basis.get_column( 1 ), angle );
                const godot::Vector3 pos = trans.origin + radius * normal;

                const godot::Vector2 uv( ak * uv_x_max, distance_along_path * uv_scale.y );

                const TG_Tangents t = get_tangents_from_axes(
                    normal, trans.basis.get_column( 1 ),
                    trans.basis.get_column( 2 ).rotated( trans.basis.get_column( 1 ), angle ) );
            ...
            if ( transform_index > 0 )
            {
                ...
                {
                    const int prev_ring_begin =
                        static_cast<int>( surface.positions.size() ) - 2 * ( point_count + 1 );
                    connect_strips_with_same_point_count( surface.indices, prev_ring_begin,
                                                          point_count + 1 );
                }
                else
                {
                    const int ring_begin =
                        static_cast<int>( surface.positions.size() ) - point_count - 1;
                    const int prev_ring_begin = ring_begin - previous_ring_point_count - 1;
                    connect_strips_with_different_point_count( surface.indices, prev_ring_begin,
                                                               previous_ring_point_count + 1,
    ...
    // Cap
    {
        ...
        add_cap( surface, base_surface, transforms.back(), previous_ring_point_count + 1,
                 end_cap_flat, radii.back() );
    }
}

static void transform_positions( std::vector<godot::Vector3> &positions, size_t begin_index,
                                 size_t count, const godot::Transform3D &transform )
{
...
static void transform_normals( std::vector<godot::Vector3> &normals, size_t begin_index,
                               size_t count, const godot::Basis &basis )
{
...
static void transform_tangents( std::vector<TG_Tangents> &tangents, size_t begin_index,
                                size_t count, const godot::Basis &basis )
{
...
static void offset_indices( std::vector<int> &indices, size_t begin_index, size_t count,
                            int offset )
{
    const size_t end = begin_index + count;
    for ( size_t i = begin_index; i < end; ++i )
...
static void combine_mesh_surfaces( const TG_NodeInstance &node_instance,
                                   std::vector<TG_SurfaceData> &surfaces_per_material,
                                   const godot::Transform3D &parent_transform )
{
    ...
    for ( size_t surface_index = 0; surface_index < node_instance.surfaces.size(); ++surface_index )
    {
        ...
        transform_positions( dst_surface.positions, vertices_begin_index, vertices_count,
                             transform );
        transform_normals( dst_surface.normals, vertices_begin_index, vertices_count,
                           transform.basis );
        transform_tangents( dst_surface.tangents, vertices_begin_index, vertices_count,
                            transform.basis );
        offset_indices( dst_surface.indices, indices_begin_index, indices_count,
                        static_cast<int>( vertices_begin_index ) );
                                ...
                                godot::RandomNumberGenerator &rng )
{
    ...
    TG_SurfaceData &surface =
        get_or_create_surface( node_instance.surfaces, leaf_params.getMaterialIndex() );
    ...
    {
        scale +=
            leaf_params.getScaleJitter() * rng.randf_range( -leaf_params.getScale(), leaf_params.getScale() );
    }
...
void TG_Tree::process_node( const TG_Node &node, TG_NodeInstance &node_instance,
                            godot::Vector3 sun_dir_local, godot::RandomNumberGenerator &rng )
{
    ...
    for ( int i = 0; i < node.get_child_count(); ++i )
    {
        ...
        for ( size_t j = 0; j < spawns.size(); ++j )
        {
            ...
            const godot::Transform3D path_trans =
                interpolate_path( node_instance.path, node_instance.path_distances, offset );
            ...
            process_node(
                ...
                child_node_instance->local_transform.basis.inverse().xform( sun_dir_local ),
                **child_rng );
...
void TG_Tree::generate_node_path( const TG_Node &node, TG_NodeInstance &node_instance,
                                  godot::Vector3 sun_dir_local, godot::RandomNumberGenerator &rng )
{

    const TG_SpawnParams &spawn_params = node.get_spawn_params();

    const float relative_offset_ratio = ( node_instance.offset_ratio - spawn_params.along_begin_ratio ) / ( spawn_params.along_end_ratio - spawn_params.along_begin_ratio );
                               ...
                               godot::RandomNumberGenerator &rng, float parent_length )
{
    ...
    const int amount =
        params.along_base_amount + static_cast<int>( params.along_amount_per_unit * parent_length );
    ...
    const float k_along_jitter = 0.5f * params.along_jitter *
                                 ( params.along_end_ratio - params.along_begin_ratio ) /
    ...
    for ( int i = 0; i < amount; ++i )
    {
        ...
        for ( int j = 0; j < params.around_amount; ++j )
        {
            ...
            const float v_angle = params.vertical_angle + params.vertical_angle_jitter *
                                                              rng.randf_range( -half_pi, half_pi );
            ...
            basis = basis.rotated( godot::Vector3( 0, 1, 0 ),
                                   af * static_cast<float>( Math_TAU ) + params.around_offset );
