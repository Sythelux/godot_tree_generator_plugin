...
namespace Zylann::TreeGen
{
    ...
    class TG_Tree : public Node3D
    {
        GDCLASS( TG_Tree, Node3D )
    public:
        TG_Tree();
        ~TG_Tree() override;
    ...
    private:
        void process_node( const TG_Node &node, TG_NodeInstance &node_instance, godot::Vector3 sun_dir_local, godot::RandomNumberGenerator &rng );

        void generate_node_path( const TG_Node &node, TG_NodeInstance &node_instance, godot::Vector3 sun_dir_local, godot::RandomNumberGenerator &rng );
        ...
        static void generate_spawns( std::vector<TG_Tree::SpawnInfo> &transforms, const TG_SpawnParams &params, godot::RandomNumberGenerator &rng, float parent_length );
